...
def solve(p, y_ref, alg_opts):
    ...
    ck = jnp.abs(ck)  # ensure non-negative coefficients

    xk, sk = p.sample_param(len(ck))
    sk = jnp.ones_like(sk) * 0.01
    ...
    pad_size = xk.shape[0]
    hessian_ck_2 = 2 * jnp.ones((pad_size,))
    ...
    for k in range(1, max_step  + 1):
        ...
        # assert jnp.all(jnp.abs(dz[int(jnp.sum(suppc)):]) < 1e-14), "dz contains non-zero values in the lower trunk, check the problem setup."
