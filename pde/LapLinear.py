...
class LapGaussianHighDim(GaussianKernel):
    """
    ...
    def DE_kappa(self, x, s, xhat, *args):
        diff = x - xhat                      # (d,)
        r_squared = jnp.sum(diff ** 2)              # scalar

        sigma = self.sigma(s).squeeze()      # scalar

        temp = (r_squared - self.d * sigma**2) / (sigma**4)

        phi = self.kappa(x, s, xhat)         # scalar
        return phi * temp                   # scalar: \Delta k
...
class LapMaternHighDim(MaternKernel):
    """
    ...
    def DE_kappa(self, x, s, xhat, *args):
        ...
        lap_phi = self._radial_laplacian(r, ell)
...
def f_sum_low_freq(x):
    ...
    """
    return -(jnp.pi**2 / 16.0) * ex_sol_sum_low_freq(x)
...
def _laplacian_rpow4d_single(x, q=0.6):
    """
    Compute Δu(x) = trace(Hess u(x)) for u(x)=c*r(x)^q (your current manufactured u).
    """
    def u_fun(z):
...
class PDE:
    ...
    KERNEL_REGISTRY: Dict[str, Callable[[dict, "PDE"], Any]] = {
        "gaussian": lambda kcfg, p: LapGaussianHighDim(
            d=p.d,
            power=p.power,
        ...
        'matern52': lambda kcfg, p: LapMaternHighDim(
            d=p.d,
            power=p.power,
            nu=kcfg.get("nu", 2.5),
            sigma_max=kcfg.get("sigma_max", 1.0),
    ...
    def _build_exact_sol_rhs(self, ex_sol_key, pcfg=None):
        if ex_sol_key == 'rpow4d':
            if pcfg is not None:
                ...
                q = 0.6
            print(q)
            self.f = partial(f_rpow4d_lap, q=q)
