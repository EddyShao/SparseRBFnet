...
class BiLapGaussianHighDim(GaussianKernel):
    ...
    def __init__(
        ...
        self.linear_E = (self.BiLap_kappa_X_c_Xhat,)
    ...
    def E_kappa_X_c_Xhat(self, *linear_results):
        # linear_results = (BiLap_kappa_X_c_Xhat,)
        bilap_u_vals, = linear_results
        return bilap_u_vals 
...
def f_sum_low_freq(x):
    ...
    RHS f(x) = -Δu(x) = (pi^2 / 4) * sum_{i=1}^d sin(pi/2 * x_i)
             = (pi^2 / 4) * u(x)
    """
    return (jnp.pi**4 / 4**4) * ex_sol_sum_low_freq(x)
...
def f_sum(x):
    ...
    RHS f(x) = -Δu(x) = (pi^2 / 4) * sum_{i=1}^d sin(pi/2 * x_i)
             = (pi^2 / 4) * u(x)
    """
    return (jnp.pi**4 / 16.0) * ex_sol_sum(x)
