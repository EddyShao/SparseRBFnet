...
class BiLapWendlandHighDim(WendlandKernel):
    ...
    def _radial_laplacian(self, r, sigma):
        ...
        # limit r → 0: Δφ(0) = d * φ''(0)
        # limit r → 0: Δφ(0) = d * φ''(0)
        _, phi2_0, _, _ = self._phi_radial_derivs(0.0, sigma)
        lap0 = d * phi2_0
    ...
    def _radial_bilaplacian(self, r, sigma):
        ...
        # Δ²u(0)  = d(d+2)/3 * u''''(0)
        # Δ²u(0)  = d(d+2)/3 * u''''(0)
        _, _, _, phi4_0 = self._phi_radial_derivs(0.0, sigma)
        bilap0 = d * (d + 2.0) / 3.0 * phi4_0
...
def f_sum_low_freq(x):
    ...
    RHS f(x) = -Δu(x) = (pi^2 / 4) * sum_{i=1}^d sin(pi/2 * x_i)
             = (pi^2 / 4) * u(x)
    """
    return (jnp.pi**4 / 4**4) * ex_sol_sum_low_freq(x) + ex_sol_sum_low_freq(x) ** 3

def b_sum_low_freq(x):
    return ex_sol_sum_low_freq(x)

def b_sum_aux_low_freq(x):
    d = x.shape[-1]
    return - (jnp.pi**2 / 4**2) * ex_sol_sum_low_freq(x)
