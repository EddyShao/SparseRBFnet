...
BASE_DIR = "exps/exp_Eikonal_1D/"    # change this if needed
...
def aggregate_one_pkl(path: str) -> Dict[str, Any]:
    ...
    # Try common variants for L-infinity test key
    L_inf = (
        s_last.get("L_inf_test", None)
        or s_last.get("l_inf_test", None)
        or s_last.get("Linf_test", None)
        or s_last.get("l_inf", None)
        or s_last.get("L_inf", None)
    )
...
def main():
    ...
    for exp_name, runs in all_results.items():
        ...
        linfs = [r["L_inf_test"] for r in runs if r["L_inf_test"] is not None]
