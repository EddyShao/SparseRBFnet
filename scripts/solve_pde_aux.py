...
def evaluate_solution(
    ...
    *,
    ...
    rhs_test_bnd = p.b(p.test_bnd)
    rhs_test_bnd_aux = p.b_aux(p.test_bnd)
    rhs_test = jnp.concatenate((rhs_test_int, rhs_test_bnd, rhs_test_bnd_aux), axis=0)

    # Objective for test set (uses same scale as training, or override)
    p.obj_test = Objective(
        p.test_int.shape[0],
        2 * p.test_bnd.shape[0],
...
def main():
    ...
    rhs = rhs.at[-p.Nx_bnd:].set(p.b(p.xhat_bnd))
    rhs_aux = p.b_aux(p.xhat_bnd)
    rhs = jnp.concatenate((rhs, rhs_aux), axis=0)
